...
export default function Login() {
  ...
  // function delModels(){
  //   const stringKeyArray = Object.keys(combined_model.models)
  //   const db = getDatabase();
  //   for (const k of stringKeyArray) {
  //     delete combined_model.models[k];
  //     num = Object.keys(combined_model.models).length;
  //     remove(ref(db, 'submitions/'+k));
  //   }
  //   ListItems()
  //   setre(0);
  // }
  ...
  const changed = (_event) => {
    console.log(combined_model.models)
    if (_event.target.checked === true){
      ...
      delete combined_model.models[_event.target.value];
      num= Object.keys(combined_model.models).length;
    }
  };
      ...
      <RootStyle>
        ...
        {/*{mdUp && (*/}
          <SectionStyle>
            ...
            <Button variant="outlined" sx={{ width: 100, mx: "auto", mb: 2 }} onClick={() => {resetCombined(); setre(re+1); }}>
            Refresh
            </Button>
            <Button variant="outlined" sx={{ width: 100, mx: "auto", mb: 2 }} onClick={() => {saveSvg();}}>
              Download
            </Button>
            {/*<Button variant="outlined" sx={{ width: 100, mx: "auto"}} onClick={delModels()}>*/}
            {/*  Delete*/}
            {/*</Button>*/}
            <Button variant="outlined" sx={{ width: 100, mx: "auto", mb: 2 }} onClick={
            () => {
              const db = getDatabase();
              console.log(ref(db)._path)
